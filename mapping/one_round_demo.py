...
# ---------- 固定模型 key（避免随 LM Studio 列表顺序漂移）----------
# Step 1 & Step 4：场景生成 + 反馈打分，用主模型（长上下文 + 强推理）
FIXED_MAIN_MODEL_KEY = "glm-4.7"
# Step 2：标签选择，用更轻量的 Qwen 标签专用模型
FIXED_TAG_MODEL_KEY = "qwen2.5-7b-instruct-mlx"
...
请先根据第一步锁定的大类（樊笼 / 沉浮 / 尘缘 / 方寸），只在该大类对应的 3 个子标签里做选择：

- 若第一步锁定为【樊笼】（职场/生存）：
  - 樊笼-形如槁木：长期泡在一个系统/组织里，身体还能撑住，但意义感几乎清零。典型感受是“干着不想干的活，心早就枯了”，像被工作流程困住、只剩机械执行。
  - 樊笼-戾气横生：对职场/系统的不公与失控感拉满，一切都想反击，处在高能对抗态。典型是“见谁都不顺眼、动不动就想怼回去”，怨气和愤怒是主轴。
  - 樊笼-疲于奔命：多线拉扯、任务堆叠导致算力分裂：到处救火、一直在赶、永远在补窟窿。典型感受是“什么都得我来”“每天像被追着跑”，重点是过载与拉扯，而不是单点不公。

- 若第一步锁定为【沉浮】（外物/得失）：
  - 沉浮-求之不得：被匮乏感与比较心驱动的抓取。典型是“别人都有我没有”“总觉得再多一点就好了”，核心是抢资源、记账、不甘心。
  - 沉浮-无常忽至：遭遇突发损失或打击（失业、破产、意外变故），整个人又惊又懵，处在否认或冻结状态。重点是“突然发生、猝不及防”的震荡。
  - 沉浮-意平难伏：现实和理想之间长期拉扯：看上去还过得去，但心里一直较劲、反复咀嚼不甘。不是瞬间的大打击，而是拉长线的内耗与不服气。

- 若第一步锁定为【尘缘】（家庭/关系）：
  - 尘缘-嗔心顿起：亲密关系中，以爱为名的控制与责怪：一言不合就情绪爆炸、翻旧账、互相伤害。典型是“我都是为你好，你怎么这样”，核心是控制和攻击。
  - 尘缘-力不从心：深陷角色/责任中，越帮越累、边界溃散。典型是“所有人的事都压到我身上”“不照顾对方就会有巨大罪恶感”，重点是拯救者情结和耗竭。
  - 尘缘-孤影自怜：在关系里长期得不到看见与回应，于是收回自己、关上门，自怜又自我否定。典型是“明明在人群里，却只觉得自己是多余的”。

- 若第一步锁定为【方寸】（自我/内省）：
  - 方寸-妄念纷飞：脑内念头过多、过快，企图“想清楚一切来换安全感”。典型是反复推演各种可能、停不下来的反刍和灾难化预演。
  - 方寸-百念皆灰：核心信念塌陷后的低能麻木：对世界和自己都提不起兴趣，觉得“什么都没意义”，像在灰色滤镜里自动行走。
  - 方寸-灵台清明：已经厌倦低维纠缠，开始主动寻找更高维的自我安放方式：不是崩溃，而是一种看穿后的疲惫与抽身。典型是“表面生活还在继续，但我在找一种更本质的活法”。

选择原则：
- 不要看词面最像什么，而是判断场景真正的底层动力：
  - 是长期枯竭、爆炸反击、还是被任务拖扯？
  - 是突发打击、慢性较劲、还是匮乏抓取？
  - 是控制与责怪、责任过载、还是没被看见？
  - 是想太多、什么都不想、还是开始抽身往上走？
- 一旦锁定一个子场景标签（例如“沉浮-求之不得”），不得回头改选，直接进入第三步（A/B/C）。
...
def main() -> int:
    ...
    # 将已加载 LLM 的 key 收集为集合，便于检查固定 key 是否存在
    ...
    # 0.1 主模型：优先使用写死的 glm-4.7，若未加载则退回旧逻辑
    if FIXED_MAIN_MODEL_KEY in loaded_llm_keys:
        model_key = FIXED_MAIN_MODEL_KEY
    else:
        model_key = get_loaded_llm_key(models)

    # 0.2 标签模型：优先使用写死的 qwen2.5-7b-instruct-mlx，若未加载则退回旧逻辑
    if FIXED_TAG_MODEL_KEY in loaded_llm_keys:
        qwen_tag_model_key = FIXED_TAG_MODEL_KEY
    else:
